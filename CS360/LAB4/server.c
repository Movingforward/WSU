...
main(int argc, char *argv[])
{
   ...
   while(1){
     ...
     while(1)
	 {
       ...
       input = strtok(line, " "); //Tokenize cmd and filename
       for(i = 0; input; i++)
	   {
		   name[i] = input;
           printf("%s\n",input);
           input = strtok(NULL, " "); //NULL to continue tokenizing same string
		  // printf("%s\n", name[i]);
       }
       ...
       if(strcmp(name[0], "get") == 0)
	   {
        ...
        if((fdI = open(to, O_RDONLY)) == -1)
        {
          printf("Failed to open file\n");
        }

        while((nC = read(fdI, buf, 1024)) > 0)
        {
          if (write(client_sock, buf, nC) != nC)
          {
            printf("Cannot Write\n");
          }
          if (nC == -1)
